...
loadActiveArchiveData()
  .then(data => {
    ...
    const totalInstrTime = instrSegments.reduce((sum, seg) => sum + (seg.end - seg.start), 0);
    ...
    instrSegments.forEach((seg, i) => {
      let width = ((seg.end - seg.start) / totalInstrTime) * 100;
    ...
    function enterHoverZone() {
      hoverCount++;
      setTimelineRowVisibility(true);
    }

    function leaveHoverZone() {
      hoverCount--;
      if (hoverCount <= 0 && !isDragging) {
        setTimelineRowVisibility(false);
      }
    }

    [timelineZone, cursorWrapper].forEach(el => {
      el.addEventListener("mouseenter", enterHoverZone);
      el.addEventListener("mouseleave", leaveHoverZone);
    });
  })
  .catch(err => {
